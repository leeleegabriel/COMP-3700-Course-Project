...
public class StoreServer extends Thread {
   ...
   private Object processRequest(ClientRequest request) throws Exception {
      ...
      if (request.getCommand().equals("GET Row"))
         return dataAdapter.getRow();
      
      if (request.getCommand().equals("GET Order"))
         return dataAdapter.loadOrder(Integer.parseInt(request.getData()));
      if (request.getCommand().equals("PUT Order")) {
         Order order = gson.fromJson(request.getData(), Order.class);
         if (dataAdapter.saveOrder(order))   // store successfully to the database
            return "OK";
         else
            return "ERROR";
      }  
         
      if (request.getCommand().equals("PUT UserUpdate")) {
         System.out.println(gson.fromJson(request.getData(), User.class));
         return "OK";     
      }
      
      if (request.getCommand().equals("PUT User")) {
         User user = gson.fromJson(request.getData(), User.class);
         if (dataAdapter.saveUser(user))   // store successfully to the database
            return "OK";
         else
            return "ERROR";
      }
      if (request.getCommand().equals("GET User")) {
         ...
         return dataAdapter.getUser(username);  
      }
